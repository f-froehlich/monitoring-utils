...
class SpaceIO(Plugin):
    ...
    def add_args(self):
        ...
        self.__parser.add_argument('-v', '--volume', dest='volume', required=True,
                                   type=int, help='Number of the volume to check')
    ...
    def run(self):
        ...
        if 0 == len(status_data):
            self.__status_builder.unknown(
                Output(f'Volume "{self.__volume_id}" either does not exist or not accessible'))
            self.__status_builder.exit()
        ...
        for status_data_id in status_data:
            ...
            output = Output(f"Volume \"{data['device']}\" [{data.get('uuid', 'unknown UUID')}]", perfdata)
