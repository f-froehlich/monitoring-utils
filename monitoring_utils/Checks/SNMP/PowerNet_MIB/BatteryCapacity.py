...
class BatteryCapacity(Plugin):
    ...
    def add_args(self):
        ...
        self.__parser.add_argument('-w', '--warning', dest='warning', required=False,
                                   type=int, help='Warning Capacity', default=50)
        self.__parser.add_argument('-c', '--critical', dest='critical', required=False,
                                   type=int, help='Critical Capacity', default=20)
    ...
    def run(self):
        ...
        if 1 != len(oids) or '0' != oids[0]['oid']:
            self.__status_builder.unknown(Output(f'Couldn\'t get Battery capacity information'))
            return

        capacity = oids[0]['value']

        output = Output(f'Capacity is {capacity}%',
                        [Perfdata('capacity', capacity, unit='%', warning=self.__warning,
                                  critical=self.__critical, min=0, max=100)])
