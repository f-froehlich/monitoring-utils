...
class SystemdServiceEnabled(Plugin):
    ...
    def __init__(self):
        ...
        Plugin.__init__(self, 'Check if a systemd service is enabled')
    ...
    def run(self):
        cli_executor = CLIExecutor(logger=self.__logger, status_builder=self.__status_builder,
                                   command_array=['systemctl', 'is-enabled', self.__unit_name])
        ...
        elif 'enabled' != output[0].lower():
            self.__status_builder.critical(f'The unit {self.__unit_name} is not enabled.')
        else:
            self.__status_builder.success(f'The unit {self.__unit_name} is enabled.')
